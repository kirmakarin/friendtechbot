...
const TWITTER_SCOUT_SCORE = 500; // Adjust this number as per your requirement.
const TWITTER_SCOUT_PERFECT_SCORE = 1000; // Adjust this number as per your requirement.
const MAX_BUY_PRICE = 25000000;
const balanceArray = [];
const alreadyCheckedKeys = [];
...
const run = async () => {
  let filter = friends.filters.Trade(
    null,
    null,
    null,
    null,
  ...
  friends.on(filter, async (event) => {
    ...
    if (details.isBuy && details.supply < 6) {
      ...
      const lastMessageTime = new Date(userData.lastMessageTime);
      const twentyFourHoursAgo = new Date(
        new Date().getTime() - 24 * 60 * 60 * 1000,
      );

      if (lastOnline < twentyFourHoursAgo) return;

      let buyPrice = await friends.getBuyPriceAfterFee(amigo, 1);
      const myBalance = await provider.getBalance(
        "0xDC94d0C56285E70274f7C5e14670CcAb28A9D677",
      );
      if (buyPrice >= myBalance) return;
      ...
      if (qty > 0) {
        ...
        if (
          (qty < 2 && buyPrice > 2000000000000000) ||
        ...
        console.log(
          `User ${amigo} (${twitterUsername}) does not meet the follower count requirement.`,
        );
      }
    }
  });
...
async function twitterCheck(twitterUsername, followerNumber) {
  ...
  if (!twitterUsername.startsWith("0x")) {
    ...
    if (
      followers_count > followerNumber &&
      statuses_count > 50 &&
...
async function getUserData(address) {
  try {
    ...
    console.error(
      `Failed to fetch user data for address ${address}: ${err.message}`,
    );
  }
  return { twitterUsername: address, lastOnline: 0, lastMessageTime: 0 };
}
...
async function getTwitterUserData(profileName) {
  ...
  try {
    const response = await fetch(
      `https://api.twitter.com/1.1/users/lookup.json?screen_name=${profileName}`,
      requestOptions,
    );
    if (response.ok) {
      return await response.json();
    ...
    console.error(
      `Failed to fetch twitter user data for ${profileName}: ${err.message}`,
    );
  }
  return 0;
}
...
async function getTweetScoutData(profileName) {
  ...
  try {
    const response = await fetch(
      `http://77.232.42.221:52354/api/score/${profileName}`,
      requestOptions,
    );
    if (response.ok) {
    ...
    console.error(
      `Failed to fetch tweet scout user data for ${profileName}: ${err.message}`,
    );
  }
  return 0;
}
...
const syncWallets = async () => {
  fs.readFile("./buys.txt", "utf-8", (err, data) => {
    if (err) {
      console.error("Error reading the file:", err);
      return;
    }

    // Split the file contents into an array based on newlines
    const valuesArray = data.split("\n").filter(Boolean); // Remove empty lines
    alreadyCheckedKeys.push(valuesArray);

    // Now, valuesArray contains the values from the file
    console.log("Values from the file:", valuesArray);
  });
};

const fillAlreadyCheckedKeys = async () => {
  if (alreadyCheckedKeys.indexOf(event.args[1]) !== -1) {
    return;
  }
  alreadyCheckedKeys.push(event.args[1]);
};

const botCheck = async () => {
  for (const botBalance in balanceArray) {
    if (
      weiBalance > botBalance - 300000000000000 &&
      weiBalance < botBalance + 300000000000000
    ) {
      return;
    }
